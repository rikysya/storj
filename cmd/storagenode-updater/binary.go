...
func downloadBinary(ctx context.Context, url, target string) error {
	...
	zap.S().Infof("Start downloading %s to %s", url, f.Name())
	...
	zap.S().Infof("Finished downloading %s to %s", url, f.Name())
	return nil
}
...
func unpackBinary(ctx context.Context, archive, target string) (err error) {
	...
	if len(zipReader.File) != 1 {
		return errs.New("archive should contain only binary file")
	}
